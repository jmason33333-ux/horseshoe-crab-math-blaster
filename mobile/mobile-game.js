/* =============================================
   MOBILE-GAME.JS — Portrait touch-first controller
   Shares equations.js, balloons.js, obstacles.js, scoring.js
   ============================================= */

'use strict';

/* ---- CANVAS SETUP ---- */
const canvas = document.getElementById('gameCanvas');
const ctx = canvas.getContext('2d');

const HUD_H       = 48;  // must match CSS #hud height
const TOUCH_CTL_H = 80;  // must match CSS #touchControls height

function resizeCanvas() {
  canvas.width  = window.innerWidth;
  canvas.height = window.innerHeight - HUD_H - TOUCH_CTL_H;
}
resizeCanvas();
window.addEventListener('resize', resizeCanvas);

/* ---- MOBILE FISH SPEED PATCH ----
   On narrow mobile screens the fish cross too fast.
   Fix: lock all levels to the same base speed (no level scaling)
   and scale vx down by MOBILE_SPEED_FACTOR after each spawn.
   ----------------------------------------- */
const MOBILE_SPEED_FACTOR = 0.65; // ~35% slower, flat across all levels
(function patchMobileFishSpeed() {
  const _orig = BalloonManager.spawnEquationBalloons.bind(BalloonManager);
  BalloonManager.spawnEquationBalloons = function(equation, level, w, h) {
    _orig(equation, 1, w, h);          // always pass level=1 → no speed scaling
    BalloonManager.all.forEach(f => {  // then scale vx down for mobile
      f.vx *= MOBILE_SPEED_FACTOR;
    });
  };
})();

/* ---- GAME STATE ---- */
const GameState = {
  MENU: 'menu',
  NAME: 'name',
  LEVEL_INTRO: 'level_intro',
  PLAYING: 'playing',
  LEVEL_COMPLETE: 'level_complete',
  REPORT: 'report',
  HIGH_SCORES: 'high_scores',
};

let currentState = GameState.MENU;
let currentLevel = 1;
const TOTAL_LEVELS   = 10;
const ROUND_DURATION = 120; // seconds

/* ---- CRAB ---- */
const crab = {
  x: 0,
  y: 0,
  width: 56,   // slightly smaller than desktop for portrait
  height: 40,
  speed: 260,  // px/sec — comfortable on narrow screens
};

/* ---- TOUCH MOVE STATE ---- */
// True while the left/right button is held down
const touchMove = { left: false, right: false };

/* ---- SPINES ---- */
const spines = [];
const SPINE_SPEED   = 460;
const SPINE_COOLDOWN = 0.30;
let spineCooldown = 0;

/* ---- GAME LOOP ---- */
let lastTime   = 0;
let gameTimer  = ROUND_DURATION;
let animFrameId = null;

/* ---- EQUATION ---- */
let currentEquation  = null;
let equationPending  = false;
let equationDelay    = 0;
const EQUATION_DELAY = 1.0;

/* ---- SCORE FLASH ---- */
let scoreFlash = null;

/* ---- BACKGROUND WAVES ---- */
const waves = Array.from({ length: 5 }, (_, i) => ({
  y:      0.2 + i * 0.15,
  offset: Math.random() * Math.PI * 2,
  speed:  0.3 + i * 0.09,
  amp:    10 + i * 3,
  color:  `rgba(0, ${90 + i * 15}, ${160 + i * 10}, ${0.18 - i * 0.02})`,
}));
let waveTime = 0;

/* ---- PARTICLES ---- */
let particles = [];

/* =============================================
   SCREEN HELPERS
   ============================================= */
function showScreen(id) {
  document.querySelectorAll('.screen').forEach(s => s.classList.remove('active'));
  document.getElementById(id).classList.add('active');
}

/* =============================================
   MAIN MENU
   ============================================= */
document.getElementById('startBtn').addEventListener('click', () => {
  showScreen('nameScreen');
  currentState = GameState.NAME;
  document.getElementById('playerName').focus();
});

document.getElementById('highScoreBtn').addEventListener('click', () => {
  document.getElementById('highScoreList').innerHTML = ScoringEngine.renderHighScoresHTML();
  showScreen('highScoreScreen');
  currentState = GameState.HIGH_SCORES;
});

document.getElementById('backFromHighScore').addEventListener('click', () => {
  showScreen('mainMenu');
  currentState = GameState.MENU;
});

/* =============================================
   NAME SCREEN
   ============================================= */
document.getElementById('nameConfirmBtn').addEventListener('click', startGameFromName);
document.getElementById('playerName').addEventListener('keydown', e => {
  if (e.key === 'Enter') startGameFromName();
});

function startGameFromName() {
  const name = document.getElementById('playerName').value.trim() || 'Player';
  ScoringEngine.init();
  ScoringEngine.setPlayer(name);
  document.getElementById('hudName').textContent = name;
  currentLevel = 1;
  showLevelIntro(currentLevel);
}

/* =============================================
   LEVEL INTRO
   ============================================= */
function showLevelIntro(level) {
  currentState = GameState.LEVEL_INTRO;
  const mathLabel  = EquationEngine.getLevelLabel(level);
  const balloons   = EquationEngine.getBalloonCount(level);
  const pointsPerQ = [0, 2, 2, 4, 4, 6, 6, 8, 8, 10, 10][level];

  document.getElementById('levelIntroContent').innerHTML = `
    <h2>Level ${level} of ${TOTAL_LEVELS}</h2>
    <p class="level-detail">Math focus:</p>
    <div class="math-type">${mathLabel}</div>
    <p class="level-detail">🐠 ${balloons} fish per question</p>
    <p class="level-detail">⭐ ${pointsPerQ} pts per correct answer</p>
    <p class="level-detail">⏱ ${ROUND_DURATION} seconds — go!</p>
    <p class="level-detail" style="margin-top:10px;color:#ffe066">
      ◀ ▶ to move &nbsp;|&nbsp; 🎯 to shoot
    </p>
  `;
  showScreen('levelIntro');
}

document.getElementById('levelStartBtn').addEventListener('click', () => {
  startLevel(currentLevel);
});

/* =============================================
   START LEVEL
   ============================================= */
function startLevel(level) {
  currentState = GameState.PLAYING;
  ScoringEngine.setLevel(level);
  EquationEngine.resetRound();
  gameTimer = ROUND_DURATION;
  spineCooldown = 0;
  spines.length = 0;
  particles.length = 0;
  BalloonManager.reset();
  ObstacleManager.reset();
  currentEquation = null;
  equationPending = true;
  equationDelay   = 0.5;

  crab.x = canvas.width / 2;
  crab.y = canvas.height - crab.height / 2 - 14;
  touchMove.left = false;
  touchMove.right = false;

  document.getElementById('levelDisplay').textContent  = level;
  document.getElementById('scoreDisplay').textContent  = ScoringEngine.getScore();
  document.getElementById('timerDisplay').textContent  = Math.ceil(gameTimer);
  document.getElementById('equationDisplay').textContent = 'Ready!';

  showScreen('gameScreen');
  tryStartMusic();

  lastTime    = performance.now();
  if (animFrameId) cancelAnimationFrame(animFrameId);
  animFrameId = requestAnimationFrame(gameLoop);
}

/* =============================================
   GAME LOOP
   ============================================= */
function gameLoop(timestamp) {
  const dt = Math.min((timestamp - lastTime) / 1000, 0.05);
  lastTime = timestamp;
  if (currentState !== GameState.PLAYING) return;
  update(dt);
  render();
  animFrameId = requestAnimationFrame(gameLoop);
}

/* =============================================
   UPDATE
   ============================================= */
function update(dt) {
  waveTime += dt;

  gameTimer -= dt;
  document.getElementById('timerDisplay').textContent = Math.ceil(Math.max(0, gameTimer));
  if (gameTimer <= 0) { endLevel(); return; }

  if (spineCooldown > 0) spineCooldown -= dt;

  updateCrab(dt);

  if (scoreFlash) {
    scoreFlash.timer -= dt;
    scoreFlash.y -= 36 * dt;
    if (scoreFlash.timer <= 0) scoreFlash = null;
  }

  particles.forEach(p => {
    p.x += p.vx * dt;
    p.y += p.vy * dt;
    p.life -= dt;
    p.vy += 70 * dt;
  });
  particles = particles.filter(p => p.life > 0);

  updateEquationFlow(dt);

  for (let i = spines.length - 1; i >= 0; i--) {
    const s = spines[i];
    s.y -= SPINE_SPEED * dt;
    if (currentEquation) {
      const hit = BalloonManager.checkHit(s.x, s.y);
      if (hit) { spines.splice(i, 1); handleBalloonHit(hit); continue; }
    }
    if (s.y < -20) spines.splice(i, 1);
  }

  ObstacleManager.update(dt, currentLevel, canvas.width, canvas.height);

  if (ObstacleManager.checkCrabHit(crab.x, crab.y, crab.width * 0.36)) {
    const delta = ScoringEngine.onObstacleHit();
    showScoreFlash(delta, crab.x, crab.y - 28, '#ff6b6b');
    updateScoreDisplay();
    spawnHitParticles(crab.x, crab.y, '#ff6b6b');
    playSound('obstacleHit');
  }

  BalloonManager.update(dt);
}

function updateCrab(dt) {
  // Touch buttons held down
  if (touchMove.left)  crab.x -= crab.speed * dt;
  if (touchMove.right) crab.x += crab.speed * dt;

  // Keyboard fallback (for tablet+keyboard users)
  if (keys['ArrowLeft'])  crab.x -= crab.speed * dt;
  if (keys['ArrowRight']) crab.x += crab.speed * dt;

  const half = crab.width / 2;
  crab.x = Math.max(half, Math.min(canvas.width - half, crab.x));
  crab.y = canvas.height - crab.height / 2 - 14;
}

/* =============================================
   EQUATION FLOW
   ============================================= */
function updateEquationFlow(dt) {
  if (equationPending) {
    equationDelay -= dt;
    if (equationDelay <= 0) {
      spawnNextEquation();
      equationPending = false;
    }
    return;
  }
  if (!currentEquation) return;
  if (BalloonManager.allGone(canvas.width)) {
    currentEquation = null;
    equationPending = true;
    equationDelay   = EQUATION_DELAY;
    document.getElementById('equationDisplay').textContent = '…';
  }
}

function spawnNextEquation() {
  currentEquation = EquationEngine.generate(currentLevel);
  BalloonManager.spawnEquationBalloons(currentEquation, currentLevel, canvas.width, canvas.height);
  document.getElementById('equationDisplay').textContent = currentEquation.question;
}

/* =============================================
   BALLOON HIT HANDLING
   ============================================= */
function handleBalloonHit(balloon) {
  if (balloon.isBomb) {
    const delta = ScoringEngine.onBombHit();
    showScoreFlash('💣 -5', balloon.x, balloon.y - 18, '#ff4400');
    updateScoreDisplay();
    spawnHitParticles(balloon.x, balloon.y, '#ff4400', 12);
    playSound('bomb');
    currentEquation = null;
    equationPending = true;
    equationDelay   = EQUATION_DELAY;
    document.getElementById('equationDisplay').textContent = '🐡 Ouch!';

  } else if (balloon.isCorrect) {
    const pts = ScoringEngine.onCorrect(currentLevel, currentEquation ? currentEquation.type : 'addition');
    showScoreFlash(`+${pts}`, balloon.x, balloon.y - 18, '#ffe066');
    updateScoreDisplay();
    spawnHitParticles(balloon.x, balloon.y, balloon.color, 10);
    playSound('correct');
    currentEquation = null;
    equationPending = true;
    equationDelay   = EQUATION_DELAY;
    document.getElementById('equationDisplay').textContent = '🐠 Correct!';

  } else {
    ScoringEngine.onMiss(currentLevel, currentEquation ? currentEquation.type : 'addition');
    showScoreFlash('Wrong!', balloon.x, balloon.y - 18, '#aaa');
    spawnHitParticles(balloon.x, balloon.y, '#aaa', 5);
    playSound('wrong');
    currentEquation = null;
    equationPending = true;
    equationDelay   = EQUATION_DELAY;
    document.getElementById('equationDisplay').textContent = '🐟 Try again!';
  }
}

function updateScoreDisplay() {
  document.getElementById('scoreDisplay').textContent = ScoringEngine.getScore();
}

/* =============================================
   END LEVEL
   ============================================= */
function endLevel() {
  cancelAnimationFrame(animFrameId);
  currentState = GameState.LEVEL_COMPLETE;

  const summary    = ScoringEngine.getLevelSummary(currentLevel);
  const isLastLevel = currentLevel === TOTAL_LEVELS;

  let feedbackMsg = '';
  if (summary.accuracy >= 80) feedbackMsg = '🌟 Excellent!';
  else if (summary.accuracy >= 60) feedbackMsg = '👍 Good job!';
  else feedbackMsg = '💪 Keep going!';

  document.getElementById('levelCompleteContent').innerHTML = `
    <h2>${isLastLevel ? '🎉 Final Level!' : `✅ Level ${currentLevel} Done!`}</h2>
    <p style="color:#ffe066;margin-bottom:12px">${feedbackMsg}</p>
    <div class="level-stat"><span class="stat-label">Score</span><span class="stat-value">${summary.score} pts</span></div>
    <div class="level-stat"><span class="stat-label">Correct</span><span class="stat-value">${summary.correct} / ${summary.attempted}</span></div>
    <div class="level-stat"><span class="stat-label">Accuracy</span><span class="stat-value">${summary.accuracy}%</span></div>
    <div class="level-stat"><span class="stat-label">Total</span><span class="stat-value">${ScoringEngine.getScore()} pts</span></div>
  `;

  const nextBtn = document.getElementById('nextLevelBtn');
  nextBtn.textContent = isLastLevel ? '📊 Report Card' : `Level ${currentLevel + 1} →`;

  playSound('levelComplete');
  showScreen('levelComplete');
}

document.getElementById('nextLevelBtn').addEventListener('click', () => {
  if (currentLevel >= TOTAL_LEVELS) {
    showReportCard();
  } else {
    currentLevel++;
    showLevelIntro(currentLevel);
  }
});

/* =============================================
   REPORT CARD
   ============================================= */
function showReportCard() {
  currentState = GameState.REPORT;
  const report = ScoringEngine.generateReportCard();
  ScoringEngine.saveHighScore();
  document.getElementById('reportContent').innerHTML = ScoringEngine.renderReportCardHTML(report);
  showScreen('reportCard');
}

document.getElementById('playAgainBtn').addEventListener('click', () => {
  showScreen('nameScreen');
  currentState = GameState.NAME;
  document.getElementById('playerName').value = '';
  document.getElementById('playerName').focus();
});

document.getElementById('menuBtn').addEventListener('click', () => {
  showScreen('mainMenu');
  currentState = GameState.MENU;
});

/* =============================================
   RENDER
   ============================================= */
function render() {
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  drawBackground();
  drawObstacles();
  drawBalloons();
  drawSpines();
  drawCrab();
  drawParticles();
  drawScoreFlash();
}

function drawBackground() {
  const grad = ctx.createLinearGradient(0, 0, 0, canvas.height);
  grad.addColorStop(0, '#001a33');
  grad.addColorStop(0.5, '#002244');
  grad.addColorStop(1, '#003366');
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  waves.forEach(w => {
    const y = w.y * canvas.height;
    ctx.beginPath();
    ctx.moveTo(0, y);
    for (let x = 0; x <= canvas.width; x += 6) {
      ctx.lineTo(x, y + Math.sin((x / canvas.width) * Math.PI * 4 + waveTime * w.speed + w.offset) * w.amp);
    }
    ctx.lineTo(canvas.width, canvas.height);
    ctx.lineTo(0, canvas.height);
    ctx.closePath();
    ctx.fillStyle = w.color;
    ctx.fill();
  });

  // Sandy bottom
  const sandGrad = ctx.createLinearGradient(0, canvas.height - 32, 0, canvas.height);
  sandGrad.addColorStop(0, 'rgba(194, 154, 108, 0.6)');
  sandGrad.addColorStop(1, 'rgba(210, 180, 140, 0.8)');
  ctx.fillStyle = sandGrad;
  ctx.fillRect(0, canvas.height - 32, canvas.width, 32);

  // Bubbles
  drawBubbles();
}

const bubbleList = Array.from({ length: 12 }, () => ({
  x: Math.random(),
  y: Math.random() * 0.8 + 0.05,
  r: 1.5 + Math.random() * 3,
  speed: 0.01 + Math.random() * 0.02,
  phase: Math.random() * Math.PI * 2,
}));

function drawBubbles() {
  bubbleList.forEach(b => {
    b.y -= b.speed * 0.016;
    if (b.y < 0) b.y = 1;
    const bx = b.x * canvas.width + Math.sin(waveTime * 0.5 + b.phase) * 6;
    const by = b.y * canvas.height;
    ctx.beginPath();
    ctx.arc(bx, by, b.r, 0, Math.PI * 2);
    ctx.strokeStyle = 'rgba(100, 200, 255, 0.35)';
    ctx.lineWidth = 1;
    ctx.stroke();
  });
}

function drawCrab() {
  ctx.save();
  ctx.translate(crab.x, crab.y);
  // Scale down slightly for portrait screen
  ctx.scale(0.82, 0.82);
  drawHorseshoeCrab(ctx);
  ctx.restore();
}

function drawHorseshoeCrab(ctx) {
  ctx.save();
  ctx.shadowColor = 'rgba(0,0,0,0.4)';
  ctx.shadowBlur = 12;
  ctx.shadowOffsetY = 5;

  const shellGrad = ctx.createRadialGradient(-8, -14, 4, 0, -8, 38);
  shellGrad.addColorStop(0, '#e8c89a');
  shellGrad.addColorStop(0.5, '#c8975a');
  shellGrad.addColorStop(1, '#8b6535');
  ctx.fillStyle = shellGrad;
  ctx.beginPath();
  ctx.arc(0, -12, 34, 0, Math.PI * 2);
  ctx.fill();

  ctx.fillStyle = 'rgba(255,255,255,0.35)';
  ctx.beginPath();
  ctx.ellipse(-10, -24, 14, 8, -0.4, 0, Math.PI * 2);
  ctx.fill();

  ctx.shadowBlur = 0; ctx.shadowOffsetY = 0;

  ctx.fillStyle = '#8b6535';
  ctx.beginPath();
  ctx.moveTo(-4, 20); ctx.lineTo(4, 20);
  ctx.lineTo(2, 44);  ctx.lineTo(0, 48); ctx.lineTo(-2, 44);
  ctx.closePath(); ctx.fill();

  const abdGrad = ctx.createRadialGradient(-4, 12, 2, 0, 14, 22);
  abdGrad.addColorStop(0, '#d4a870');
  abdGrad.addColorStop(1, '#9a7040');
  ctx.fillStyle = abdGrad;
  ctx.beginPath();
  ctx.ellipse(0, 14, 22, 14, 0, 0, Math.PI * 2);
  ctx.fill();

  const legPositions = [-26, -18, -10];
  const legAngles    = [2.4, 2.0, 1.7];
  [-1, 1].forEach(side => {
    legPositions.forEach((lx, i) => {
      const angle  = side === -1 ? Math.PI - legAngles[i] : legAngles[i];
      const startX = side * 28;
      const startY = -5 + i * 9;
      const endX   = startX + Math.cos(angle) * 22;
      const endY   = startY + Math.sin(angle) * 10;
      ctx.beginPath();
      ctx.moveTo(startX, startY);
      ctx.quadraticCurveTo(startX + side * 14, startY + 4, endX, endY);
      ctx.strokeStyle = '#7a5528';
      ctx.lineWidth = 4; ctx.lineCap = 'round'; ctx.stroke();
      ctx.beginPath();
      ctx.arc(endX, endY, 3, 0, Math.PI * 2);
      ctx.fillStyle = '#5a3a10'; ctx.fill();
    });
  });

  const eyeY = -20;
  [-10, 10].forEach(ex => {
    ctx.fillStyle = '#fff';
    ctx.beginPath(); ctx.arc(ex, eyeY, 7, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#3d2200';
    ctx.beginPath(); ctx.arc(ex + 1, eyeY + 1, 4.5, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.beginPath(); ctx.arc(ex + 2.5, eyeY - 1.5, 1.8, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = 'rgba(255, 150, 130, 0.45)';
    ctx.beginPath(); ctx.ellipse(ex + (ex > 0 ? 4 : -4), eyeY + 6, 5, 3, 0, 0, Math.PI * 2); ctx.fill();
  });

  ctx.strokeStyle = '#5a3a10'; ctx.lineWidth = 2; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.arc(0, -10, 8, 0.2, Math.PI - 0.2); ctx.stroke();

  ctx.restore();
}

function drawSpines() {
  spines.forEach(s => {
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(s.x, s.y + 10);
    ctx.lineTo(s.x - 3, s.y + 5);
    ctx.lineTo(s.x, s.y - 10);
    ctx.lineTo(s.x + 3, s.y + 5);
    ctx.closePath();
    const spineGrad = ctx.createLinearGradient(s.x, s.y - 10, s.x, s.y + 10);
    spineGrad.addColorStop(0, '#ffe8a0');
    spineGrad.addColorStop(1, '#c87820');
    ctx.fillStyle = spineGrad; ctx.fill();
    ctx.strokeStyle = '#8b5c10'; ctx.lineWidth = 1; ctx.stroke();
    ctx.shadowColor = '#ffcc44'; ctx.shadowBlur = 6; ctx.fill();
    ctx.restore();
  });
}

function drawBalloons() { BalloonManager.draw(ctx); }
function drawObstacles() { ObstacleManager.draw(ctx); }

function drawParticles() {
  particles.forEach(p => {
    ctx.save();
    ctx.globalAlpha = p.life / p.maxLife;
    ctx.fillStyle = p.color;
    ctx.beginPath();
    ctx.arc(p.x, p.y, p.r * (p.life / p.maxLife), 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  });
}

function drawScoreFlash() {
  if (!scoreFlash) return;
  ctx.save();
  ctx.globalAlpha = Math.min(1, scoreFlash.timer / 0.6);
  ctx.fillStyle = scoreFlash.color;
  ctx.font = 'bold 22px Arial';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.shadowColor = 'rgba(0,0,0,0.6)';
  ctx.shadowBlur = 4;
  ctx.fillText(scoreFlash.text, scoreFlash.x, scoreFlash.y);
  ctx.restore();
}

/* =============================================
   HELPERS
   ============================================= */
function showScoreFlash(text, x, y, color) {
  scoreFlash = { text: String(text), x, y, timer: 1.0, color };
}

function spawnHitParticles(x, y, color, count = 8) {
  for (let i = 0; i < count; i++) {
    const angle = Math.random() * Math.PI * 2;
    const speed = 50 + Math.random() * 100;
    particles.push({
      x, y,
      vx: Math.cos(angle) * speed,
      vy: Math.sin(angle) * speed - 35,
      r: 3 + Math.random() * 4,
      color,
      life: 0.45 + Math.random() * 0.35,
      maxLife: 0.8,
    });
  }
}

/* =============================================
   FIRE SPINE
   ============================================= */
function fireSpine() {
  if (spineCooldown > 0) return;
  spines.push({ x: crab.x, y: crab.y - crab.height / 2 });
  spineCooldown = SPINE_COOLDOWN;
  playSound('shoot');
}

/* =============================================
   TOUCH CONTROLS
   ============================================= */

// Shoot button: fire on tap
const btnShoot = document.getElementById('btnShoot');
btnShoot.addEventListener('touchstart', e => {
  e.preventDefault();
  if (currentState === GameState.PLAYING) fireSpine();
}, { passive: false });
btnShoot.addEventListener('mousedown', e => {
  if (currentState === GameState.PLAYING) fireSpine();
});

// Left button: move while held
const btnLeft = document.getElementById('btnLeft');
btnLeft.addEventListener('touchstart', e => {
  e.preventDefault(); touchMove.left = true;
}, { passive: false });
btnLeft.addEventListener('touchend',   e => { e.preventDefault(); touchMove.left = false; }, { passive: false });
btnLeft.addEventListener('touchcancel',e => { touchMove.left = false; });
btnLeft.addEventListener('mousedown',  () => { touchMove.left = true; });
btnLeft.addEventListener('mouseup',    () => { touchMove.left = false; });
btnLeft.addEventListener('mouseleave', () => { touchMove.left = false; });

// Right button: move while held
const btnRight = document.getElementById('btnRight');
btnRight.addEventListener('touchstart', e => {
  e.preventDefault(); touchMove.right = true;
}, { passive: false });
btnRight.addEventListener('touchend',   e => { e.preventDefault(); touchMove.right = false; }, { passive: false });
btnRight.addEventListener('touchcancel',e => { touchMove.right = false; });
btnRight.addEventListener('mousedown',  () => { touchMove.right = true; });
btnRight.addEventListener('mouseup',    () => { touchMove.right = false; });
btnRight.addEventListener('mouseleave', () => { touchMove.right = false; });

/* =============================================
   KEYBOARD FALLBACK (tablet + keyboard)
   ============================================= */
const keys = {};
window.addEventListener('keydown', e => {
  keys[e.key] = true;
  if (e.key === ' ' && currentState === GameState.PLAYING) {
    e.preventDefault(); fireSpine();
  }
});
window.addEventListener('keyup', e => { keys[e.key] = false; });

/* =============================================
   SOUND
   ============================================= */
let audioCtx       = null;
let musicPlaying   = false;
let musicStarted   = false;

function getAudioCtx() {
  if (!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)();
  return audioCtx;
}

function playSound(type) {
  try {
    const ac = getAudioCtx();
    if (type === 'shoot') {
      beep(ac, 880, 0.08, 'square', 0.12);
    } else if (type === 'correct') {
      beep(ac, 523, 0.06, 'sine', 0.0);
      beep(ac, 659, 0.06, 'sine', 0.08);
      beep(ac, 784, 0.14, 'sine', 0.16);
    } else if (type === 'wrong') {
      beep(ac, 200, 0.18, 'sawtooth', 0.0);
    } else if (type === 'bomb') {
      beep(ac, 100, 0.3, 'sawtooth', 0.0, 0.4);
      beep(ac, 80,  0.3, 'square',   0.05, 0.3);
    } else if (type === 'obstacleHit') {
      beep(ac, 300, 0.15, 'square', 0.0, 0.2);
    } else if (type === 'levelComplete') {
      [523, 587, 659, 698, 784].forEach((f, i) => beep(ac, f, 0.12, 'sine', i * 0.1));
    }
  } catch(e) {}
}

function beep(ac, freq, duration, type = 'sine', delay = 0, freqEnd = null) {
  const osc  = ac.createOscillator();
  const gain = ac.createGain();
  osc.connect(gain);
  gain.connect(ac.destination);
  osc.type = type;
  osc.frequency.setValueAtTime(freq, ac.currentTime + delay);
  if (freqEnd) osc.frequency.exponentialRampToValueAtTime(freqEnd, ac.currentTime + delay + duration);
  gain.gain.setValueAtTime(0.16, ac.currentTime + delay);
  gain.gain.exponentialRampToValueAtTime(0.001, ac.currentTime + delay + duration);
  osc.start(ac.currentTime + delay);
  osc.stop(ac.currentTime + delay + duration + 0.01);
}

function startBackgroundMusic() {
  if (musicPlaying) return;
  try {
    const ac = getAudioCtx();
    musicPlaying = true;
    const notes = [261, 329, 392, 329, 261, 329, 392, 440,
                   349, 440, 523, 440, 349, 294, 329, 261];
    let noteIndex = 0;
    const tempo = 0.22;
    function scheduleNote() {
      if (!musicPlaying) return;
      const freq = notes[noteIndex % notes.length];
      noteIndex++;
      const osc  = ac.createOscillator();
      const gain = ac.createGain();
      osc.connect(gain);
      gain.connect(ac.destination);
      osc.type = 'triangle';
      osc.frequency.setValueAtTime(freq, ac.currentTime);
      gain.gain.setValueAtTime(0.05, ac.currentTime);
      gain.gain.exponentialRampToValueAtTime(0.001, ac.currentTime + tempo * 0.9);
      osc.start(ac.currentTime);
      osc.stop(ac.currentTime + tempo);
      setTimeout(scheduleNote, tempo * 1000);
    }
    scheduleNote();
  } catch(e) {}
}

function tryStartMusic() {
  if (!musicStarted) { musicStarted = true; startBackgroundMusic(); }
}

// Trigger music on any interaction (mobile autoplay policy)
['touchstart', 'mousedown', 'keydown'].forEach(evt =>
  document.addEventListener(evt, tryStartMusic, { once: false })
);

/* =============================================
   INIT
   ============================================= */
showScreen('mainMenu');
currentState = GameState.MENU;
